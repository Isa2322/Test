...
int main()
{
    ...
    */
    ...
    // Tomo como proceso coordinador al proceso padre
    // Creo los procesos generadores y guardo su pid en en el array pids[]
    ...
    for (i = 0; i < procesos; i++)
    {
        ...
        {
            break;
            // El proceso hijo sale del bucle para evitar crear procesos indeseados
        }
        else
        {
            // es el padre
    ...
    if (pid == 0)
    {
        ...
        while (*listos < registros)
        {
            ...
            if ((registros - *listos) >= 10)
            {
                ...
                *listos = *listos + 10;                
                ...
                for (int j = 1 ; j <= 10; j++)
                {
                    ...
                    sem_wait(buffer);
                    sem_wait(control);
                    generarPersonaBancaria(persona, auxid + j);
                    sem_post(control);
                    sem_post(ocupado);
            ...
            else if ((registros - *listos) < 10)
            {
                ...
                *listos = *listos + auxCantReg;
                sem_post(semId);

                // funcion generar registro(listos - registros)

                for (int j = 1; j <= auxCantReg; j++)
                {
                    sem_wait(buffer);
                    sem_wait(control);
                    generarPersonaBancaria(persona, auxid + j);
